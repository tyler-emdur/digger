...
// Uses artist graph traversal (related-artists) instead of the deprecated
// /recommendations and /audio-features endpoints (removed by Spotify in 2024).
...
const MIN_POPULARITY = 2;         // Floor — avoids broken/unlisted artists
...
/**
 * Generate a "why you might like this" blurb.
 * Uses genre overlap and the connection path through the user's top artists.
 */
/**
 * Generate a "why you might like this" blurb.
 * `viaGenre` is the genre search term that surfaced this artist.
 */
function generateWhyBlurb(artist, via, tasteProfile) {
  // Spotify strips genre data for unverified apps, so blurbs are connection-based
  const topNames = tasteProfile.topArtistNames || [];
  const isArtistMatch = topNames.some(n =>
    n.toLowerCase() === via?.toLowerCase() ||
    artist.name.toLowerCase().includes(n.toLowerCase())
  );

  if (isArtistMatch) {
    const matchedName = topNames.find(n => n.toLowerCase() === via?.toLowerCase()) || via;
    return `Sits in the same underground orbit as ${matchedName}.`;
  }

  const termBlurbs = {
    'bedroom pop':     'A bedroom pop gem recorded far from any label.',
    'lo fi indie':     'Lo-fi and intimate — built in a bedroom, not a studio.',
    'shoegaze':        'Washes of guitar and reverb, well below the mainstream.',
    'post punk':       'Post-punk edge with an underground following.',
    'dream pop':       'Hazy, dreamy, and completely off the radar.',
    'noise pop':       'Distortion and melody in equal measure.',
    'indie folk':      'Stripped-back indie folk from outside the algorithm.',
    'math rock':       'Intricate rhythms and unconventional song structures.',
    'emo indie':       'Emotionally raw indie with a cult underground following.',
    'experimental pop':'Pushes pop into strange, uncharted territory.',
  };

  return termBlurbs[via] || 'A hidden gem well below the mainstream radar.';
}
...
/**
 * Filter a candidate artist list: below popularity ceiling,
 * not already listened to, above minimum floor.
 */
function filterArtists(artists, listenedArtistIds, popularityThreshold) {
  return artists.filter(a =>
    a.popularity < popularityThreshold &&
    a.popularity >= MIN_POPULARITY &&
    !listenedArtistIds.has(a.id)
  );
}
...
const GENRE_BUCKET_TAGS = {
  ambient_idm:     ['idm', 'ambient', 'glitch', 'drone', 'electroacoustic', 'braindance', 'experimental electronic', 'noise music', 'musique concrete', 'microsound', 'sound art'],
  indie_folk:      ['indie folk', 'folk', 'lo-fi', 'lo fi', 'bedroom pop', 'singer-songwriter', 'emo folk', 'acoustic', 'indie pop', 'shoegaze', 'dream pop', 'bedroom indie', 'chamber pop', 'freak folk'],
  rock_alt:        ['post-punk', 'punk', 'indie rock', 'emo', 'noise rock', 'slacker rock', 'alternative', 'grunge', 'post-rock', 'math rock', 'hardcore', 'alt-emo', 'midwest emo', 'skramz'],
  electronic_club: ['dance', 'edm', 'house', 'techno', 'club', 'trance', 'hyperpop', 'digicore', 'phonk', 'bass music', 'uk garage', 'jungle', 'drum and bass', 'dnb', 'breaks', 'dubstep', 'cloud rap', 'plugg', 'sigilkore'],
  hiphop_rap:      ['hip-hop', 'hip hop', 'rap', 'trap', 'boom bap', 'lo-fi hip hop', 'underground hip hop', 'uk hip hop']
};

function classifyTagsToBucket(tags) {
  if (!tags || !tags.length) return 'other';
  const lowerTags = tags.map(t => t.toLowerCase());
  // Try each bucket in priority order
  for (const [bucket, bucketTags] of Object.entries(GENRE_BUCKET_TAGS)) {
    if (lowerTags.some(t => bucketTags.some(bt => t.includes(bt) || bt.includes(t)))) {
      return bucket;
    }
  }
  return 'other';
